...
extern "C"
{
  ...
  void setup_scr_road_app(lv_ui *ui)
  {
    ...
    lv_style_set_bg_opa(&style_road_app_main, LV_STATE_DEFAULT, 0);
  ...
  {
    if (function_0x05_app.ui->road_app_imgbtn_back == btn && event == LV_EVENT_SHORT_CLICKED)
    {
        ...
        if (list_size(&out) > 0)
        {
          ...
          for (size_t m = 0; list_size(&out); m++)
          {
            ...
            if (lnk_data.rect.w * lnk_data.rect.h > max_size)
            {
              ...
              uint8_t x = max_blobs_data.rect.x * 0.75, y = 100 + max_blobs_data.rect.y, w = (uint8_t)(max_blobs_data.rect.w * 0.75);

              self->data_cmd = { x, w };

              lv_canvas_draw_rect(zm831_ui_get_canvas(), x, max_blobs_data.rect.y * 0.75, max_blobs_data.rect.w, max_blobs_data.rect.h, &self->rect_dsc);
