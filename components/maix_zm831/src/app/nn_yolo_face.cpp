...
#define FUN_FILE_NAME_PARAM_PATH_NNPARAM    "/home/res/yolo2_face_int8.param"
#define FUN_FILE_NAME_PARAM_PATH_NNBIN      "/home/res/yolo2_face_int8.bin"
  ...
  int nn_yolo_face_app_load(zm831_home_app *app)
  {
    ...
    self->opt_param.awnn.input_num = 1;  // len(input_names)
    self->opt_param.awnn.output_num = 1; // len(output_names)

    self->opt_param.awnn.mean[0] = 127.5;
    self->opt_param.awnn.mean[1] = self->opt_param.awnn.mean[0];
    self->opt_param.awnn.mean[2] = self->opt_param.awnn.mean[0];

    self->opt_param.awnn.norm[0] = 0.0078125;
    self->opt_param.awnn.norm[1] = self->opt_param.awnn.norm[0];
    self->opt_param.awnn.norm[2] = self->opt_param.awnn.norm[0];

    self->model_path.awnn.param_path = (char *)FUN_FILE_NAME_PARAM_PATH_NNPARAM;
    self->model_path.awnn.bin_path = (char *)FUN_FILE_NAME_PARAM_PATH_NNBIN;
    ...
    self->output_buffer = (float *)malloc(self->out_fmap.w * self->out_fmap.h * self->out_fmap.c * sizeof(float));
    ...
    self->quantize_buffer = (uint8_t *)malloc(self->input.w * self->input.h * self->input.c);
    ...
    self->out_fmap.data = self->output_buffer;
    self->input.buff_quantization = self->quantize_buffer;
...
#undef FUN_FILE_NAME_PARAM_PATH_NNPARAM
#undef FUN_FILE_NAME_PARAM_PATH_N
