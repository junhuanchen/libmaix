...
// #define EVDEV_HOR_MAX 220 /*"evtest" Linux tool can help to get the correct calibraion values>*/
// #define EVDEV_VER_MIN 20
// #define EVDEV_VER_MAX 220
// #endif
    ...
    while (read(evdev_fd, &in, sizeof(struct input_event)) > 0)
    {
        ...
        else if (in.type == EV_ABS)
        {
            if (in.code == ABS_X)
                ...
                evdev_root_y = in.value;
#endif
            else if (in.code == ABS_MT_WIDTH_MAJOR)
            {
                // printf("evdev_mouse_read %d %d %d\r\n", in.type, in.code, in.value);
                evdev_button = (in.value) ? LV_INDEV_STATE_PR : LV_INDEV_STATE_REL;
                // return false;
            }
        }
